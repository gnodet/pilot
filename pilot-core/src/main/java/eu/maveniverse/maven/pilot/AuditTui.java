...
public class AuditTui extends ToolPanel {
    ...
    private void toggleRowExpand(int row, MouseEvent mouse) {
        ...
        int arrowX = lastTableInner.x() + lastHighlightWidth;
        if (view == View.VULNERABILITIES
                && row < vulnDisplayRows.size()
                && vulnDisplayRows.get(row).isGroupHeader()) {
            VulnGroup group = vulnDisplayRows.get(row).group();
            boolean allManaged = editSession != null
                    && group.rows.stream()
                            .allMatch(r -> editSession.isChanged(r.entry().ga()));
            int effectiveArrowX = allManaged ? arrowX + 2 : arrowX;
            if (mouse.x() < effectiveArrowX || mouse.x() >= effectiveArrowX + 2) return;
            group.expanded = !group.expanded;
            rebuildVulnDisplayRows();
